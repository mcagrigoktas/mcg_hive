...
class BackendManager implements BackendManagerInterface {
  IdbFactory? get indexedDB => js.context.hasProperty('window') ? window.indexedDB : WorkerGlobalScope.instance.indexedDB;

  @override
  Future<StorageBackend> open(String name, String? path, bool crashRecovery, HiveCipher? cipher, String? collection) async {
    // compatibility for old store format
    final databaseName = collection ?? name;
    final objectStoreName = collection == null ? 'box' : name;

    var db = await indexedDB!.open(databaseName, version: 1, onUpgradeNeeded: (e) {
    ...
    // update version
    if (!(db.objectStoreNames ?? []).contains(objectStoreName)) {
      print('Creating objectStore $objectStoreName in database $databaseName...');
  ...
  Future<void> deleteBox(String name, String? path, String? collection) async {
    ...
    if (collection == null) {
      ...
      final db = await indexedDB!.open(databaseName, version: 1, onUpgradeNeeded: (e) {
        var db = e.target.result as Database;
        if ((db.objectStoreNames ?? []).contains(objectStoreName)) {
  ...
  Future<bool> boxExists(String name, String? path, String? collection) async {
    ...
    try {
      ...
      if (collection == null) {
        ...
        final db = await indexedDB!.open(collection, version: 1, onUpgradeNeeded: (e) {
          var db = e.target.result as Database;
